...
3. 쪽지 전송 시 수신자 이메일 정보가 존재하지 않아 전송 실패
        ...
        배운 점
        Optional은 무조건 .get()으로 꺼내는 것이 아닌,
        상황에 따라 isEmpty(), orElseThrow(), ifPresent() 등의 안전한 방식으로 접근하는 습관이 중요함.
 
5. 카카오 로그인과 이메일 로그인의 사용자 세션 구분 문제

        문제
        로그인 기능 구현 초기에,
        카카오 소셜 로그인과 일반 이메일 로그인을 동일한 세션 구조로 처리하면서 사용자 인증 상태를 구분하기 어려웠음.
